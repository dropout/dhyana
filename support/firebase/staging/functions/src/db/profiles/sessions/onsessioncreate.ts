...
export default onDocumentCreated(
  ...
  async (event: FirestoreEvent<QueryDocumentSnapshot | undefined,
    ...
    if (event.data == null || typeof event.data == "undefined") {
      logger.error(
        "Event data is null or undefined, unable to process session stats"
      );
      return;
    } else {
    ...
    const endTimeMillisecondsSinceEpoch =
    ...
    const globalShardId =
      Math.floor(Math.random() * NUM_SHARDS).toString(10);
    const globalShardRef =
      db.doc(`global_stats/all_time/shards/${globalShardId}`);

    // Calculate UTC Date String for Global Stats
    // Assuming sessionData.endTime is a number representing
    ...
    const utcDateString = [
      ...
      String(endDateTime.getUTCMonth() + 1).padStart(2, "0"),
      String(endDateTime.getUTCDate()).padStart(2, "0"),
    ].join("-");

    // Reference for the daily stats shard
    const dailyShardId = Math.floor(Math.random() * NUM_SHARDS).toString(10);
    const globalDailyShardRef =
      db.doc(`global_stats/daily_${utcDateString}/shards/${dailyShardId}`);
    ...
    try {
      await db.runTransaction(async (transaction) => {
        ...
        const globalUpdates: Record<string, FieldValue> = {
      ...
      logger.info(
        "Successfully processed session stats for profile: " +
