...
void main() {
  ...
  setUp(() {
    ...
    when(
      () => timerAudioService.playSound(any()),
  ...
  test('plays the ending sound', () async {
    ...
    verify(
      () => timerAudioService.playSound(timerSettings.endingSound),
    ).called(1);
  });

  test(
    'stops the audio service only after the ending sound finishes playing',
    () async {
      final soundCompleter = Completer<void>();
      when(
        () => timerAudioService.playSound(any()),
      ).thenAnswer((_) => soundCompleter.future);
  ...
  test(
    'assembles a sitting timer session with a generated id and the elapsed time',
    () async {
  ...
  test(
    'derives the session start time from the elapsed time when not started',
    () async {
      ...
      final difference = result.session.startTime
  ...
  test(
    'uses the current time for the session end time when not completed',
    () async {
      final before = DateTime.now();
  ...
  test(
    'returns the timer state marked as completed with the end time set',
    () async {
