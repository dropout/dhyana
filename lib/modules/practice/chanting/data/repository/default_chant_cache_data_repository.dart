...
class DefaultChantCacheDataRepository 
  ...
  Stream<CachingProgressEntity> cacheChants(
    ...
    for (final contentId in uniqueIds) {
      ...
      final validator = ChantCacheValidator();
      final localResource = await collectLocalResource(chant);      
      final validationResult = await validator.validateChantCache(
        chant, 
    ...
    yield CachingProgressEntity(
      ...
      totalTasks: batchDownloadTask.totalTasks,
      completedTasks: batchDownloadTask.completedTasks,
      results: results.map((e) => (chant: e.chant, localResources: e.localResources))
...
/// Takes a chant and breaks it down into its audio and lyrics assets, 
  ...
  cacheChantAsset(
    ...
    final destinationPath = await buildCacheFilePath(
      contentId: chant.id,
      assetType: assetType,
      version: version,
    );
    ...
    try {
      ...
      await upsertEntry(pendingEntry);

      // Start the download task for the asset
      final downloadTask = await downloadToFile(
        storagePath: storagePath,
  ...
  collectLocalResource(ChantEntity chant) async {
    ...
    final audioPath = await buildCacheFilePath(
      contentId: chant.id,
      assetType: .audio,
      version: metadata.audioVersion,
    );
    final lyricsPath = await buildCacheFilePath(
      contentId: chant.id,
      assetType: .lyrics,
